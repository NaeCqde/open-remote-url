...
- **仮想マシン (VM) / Dockerコンテナ開発環境でのOAuthログインの苦痛を解消**
    - Dockerコンテナなどのブラウザ非搭載（またはクリーンな）環境で `wrangler login` などのCLIコマンドを実行すると、ログイン用のブラウザを起動しようとします。通常はURLをコピーしてホストのブラウザで開く必要がありますし、ログイン完了後にブラウザから `http://localhost:8976/oauth/callback` などのローカルポートに対して行われるコールバックを受け取れず、ログインが途中で失敗します。
...
- [動作原理](#動作原理)
- [環境設定](#環境設定)
    - [ホスト設定 (receiver/inactive.env)](#レシーバー設定-receiverinactiveenv)
    - [クライアント設定 (sender/inactive.env)](#センダー設定-senderinactiveenv)
- [インストール](#インストール)
- [ステータス確認](#ステータス確認)
- [アンインストール](#アンインストール)
...
| `HOST_URL`   | リモートホストデーモンのURL。`http://` と `https://`（rustls）に対応。                                                                                                         | `http://localhost:40000` |
| `SELF_URL`  | ホストがリバースプロキシのためにクライアントへ折り返す際のURL。ホスト側から到達可能なアドレスを指定（LAN IPやTailscale IP等）。こちらも`http://`と`https://`（rustls）に対応。 | `http://localhost:30000` |
...
`SCHEME` に登録したスキームのURLがクライアントOSで開かれると、**ポート転送は行わず、URLをそのままホストへ転送**してホスト側のネイティブアプリで開きます（ホスト側に対応アプリがインストールされている必要があります）。

例えば `SCHEME=vcc,unityhub` と設定した場合:

- クライアント（Windows）で `vcc://...` のリンクをクリック
- そのURLがホスト（macOS）の VCC（VRChat Creator Companion）や ALCOM で直接開く
...
_インストール完了後、クライアント側OSの設定において、「デフォルトのWebブラウザ」として **Open Remote URL Sender** を選択してください：_
