...
- `HOST_URL`: リモートホストデーモンのURL (例: `https://<host>:<port>`)
- `RELAY_HOST`: クライアントのリレーサーバーのバインドIP（デフォルト: `0.0.0.0`）
- `RELAY_PORT`: クライアントのリレーリスニングポート（デフォルト: `3000`）
