...
このツールは、**「手元のPCや仮想環境（送信側）」で開いたWebサイトを、「別のPC（受信側）」のブラウザで表示させ、必要なポートを自動でつないでくれるシステム**です。
...
1. **ブラウザ起動のインターセプト**: 送信側でデフォルトのブラウザとして登録されます。URLを開こうとするすべての動作をこのツールが一時的に受け取ります。
2. **URLをメインPCへ転送**: 受け取ったURLをメインPC（受信側）に送り、メインPCのブラウザで自動的に開きます。
3. **必要なポートの自動検出**: URLを開く15秒前までに開いていたポート、および開いてから3秒間のあいだに新しく開いたポートを自動で検知します。
4. **通り道の自動作成**: 検知したポートと同じポートをメインPC（受信側）側にも一時的に作成し、送信側へと通信を逆転送する通り道（リバースプロキシ）を自動で作ります。
5. **終わったらお片付け**: 送信側のポートが閉じられたと検知すると、メインPC側の通り道も自動で削除されます（削除されずに放置された場合も、5分後に自動消去されます）。
...
1. **OSごとの決まったフォルダ**（インストールした後はここから読み込まれます）
   - **Windows**: `%APPDATA%\open-remote-url\<senderまたはreceiver>\.env`
   - **macOS**: `Applications` フォルダ内の `OpenRemoteURLSender.app`（または `OpenRemoteURLReceiver.app`）フォルダの直下の `.env`
   - **Linux**: `~/.config/open-remote-url/<senderまたはreceiver>/.env`
2. **プログラムを実行したフォルダ**
...
### 受信側設定（メインPC側: `receiver/inactive.env`）
...
- `PASSPHRASE`: 送信側と一致させる合言葉。空欄にすると合言葉なしで動作。

### 送信側設定（サブPCや仮想環境側: `sender/inactive.env`）

```env
LISTEN=0.0.0.0:30000
HOST_URL=http://<受信側のIPアドレス>:40000
SELF_URL=http://<送信側のIPアドレス>:30000
PASSPHRASE=二人だけの合言葉
SCHEME=
HTTP=true
```

- `LISTEN`: 送信側デーモンの待ち受けアドレスとポート番号（`アドレス:ポート` の形式）
- `HOST_URL`: メインPCの住所（例: `http://<受信側のIPアドレス>:40000`）
- `SELF_URL`: メインPC（受信側）がサブPC（送信側）に折り返す際のURL。メインPC側から到達できるアドレスを指定（LAN IPやTailscale IP等。`0.0.0.0` は接続先として指定できないので注意）。
...
**カスタムURLスキームについて**: `SCHEME` に登録したスキームのURLがサブPC側で開かれると、ポート転送なしでURLをそのままメインPC（受信側）へ送り、メインPC上の対応アプリで開きます。例えば `SCHEME=vcc` と設定すれば、サブPC（Windows）で `vcc://...` のリンクをクリックしたとき、メインPC（macOS）の VCC や ALCOM で直接開きます。
...
- **Windows**: 実行ファイル（`open-remote-url-sender.exe` または `open-remote-url-receiver.exe`）をダブルクリックして、ボタン（Install Service）を押します。
- **macOS**: アプリケーションファイル（`OpenRemoteURLSender.app` または `OpenRemoteURLReceiver.app`）をダブルクリックして、ボタン（Install Service）を押します。
- **Linux**: `./install.sh` を実行します。

_インストールが終わったら、送信側の設定で「デフォルトのブラウザ（一番最初に開くブラウザ）」として **Open Remote URL** を選んでください：_
