...
### ホスト設定（メインPC側: `receiver/inactive.env`）
...
### クライアント設定（サブPCや仮想環境側: `sender/inactive.env`）

```env
LISTEN=0.0.0.0:30000
HOST_URL=http://<ホストのIPアドレス>:40000
SELF_URL=http://<クライアントのIPアドレス>:30000
PASSPHRASE=二人だけの合言葉
SCHEME=
HTTP=true
```
...
- `SELF_URL`: メインPC（ホスト）がサブPC（クライアント）に折り返す際のURL。メインPC側から到達できるアドレスを指定（LAN IPやTailscale IP等。`0.0.0.0` は接続先として指定できないので注意）。
