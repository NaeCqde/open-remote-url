...
- [工作原理](#工作原理)
- [配置说明](#配置说明)
    - [主机配置 (host/inactive.env)](#主机配置-hostinactiveenv)
    - [客户端配置 (client/inactive.env)](#客户端配置-clientinactiveenv)
- [安装说明](#安装说明)
- [状态确认](#状态确认)
- [卸载服务](#卸载服务)
- [关于项目开发](#关于项目开发)
...
| `RELAY_URL`  | 主机用于反向代理回调至客户端的 URL。必须是主机侧可访问的地址（使用 LAN IP 或 Tailscale IP 等，不可使用 `0.0.0.0`）。支持 `http://` 和 `https://`（rustls）。 | `http://localhost:30000` |
