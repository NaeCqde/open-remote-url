...
> ⚠️ **当前可用性说明**
> - **Linux 版本目前尚未经过测试。**
> - Windows 版和 macOS 版仅验证了 URL 转发功能（在主机浏览器中打开 URL）。反向代理功能（OAuth 回调转发）尚未验证。
...
| `RELAY_URL` | 主机用于反向代理回调至客户端的 URL。必须是主机侧可访问的地址（使用 LAN IP 或 Tailscale IP 等，不可使用 `0.0.0.0`）。 | `http://localhost:30000` |
