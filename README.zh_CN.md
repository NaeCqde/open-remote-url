...
| 变量 | 说明 | 默认值 |
|---|---|---|
| `LISTEN` | 绑定地址和端口（`<host>:<port>` 格式） | `0.0.0.0:4000` |
...
| 变量 | 说明 | 默认值 |
|---|---|---|
| `LISTEN` | 客户端守护进程的绑定地址和端口（`<host>:<port>` 格式） | `0.0.0.0:3000` |
| `HOST_URL` | 远程主机守护进程的 URL。支持 `http://` 和 `https://`（通过 rustls 实现，无需 OpenSSL）。 | `http://localhost:4000` |
| `RELAY_URL` | 主机用于反向代理回调至客户端的 URL。必须是主机侧可访问的地址（使用 LAN IP 或 Tailscale IP 等，不可使用 `0.0.0.0`）。 | `http://localhost:3000` |
